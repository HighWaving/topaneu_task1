...
class TopAneuVesselAwareMeanPoolDilationR1(nn.Module):
    def __init__(
        self,
        backbone: nn.Module,
    ...
    def forward(self, image: torch.Tensor, vessel_mask: torch.Tensor) -> torch.Tensor:
        ...
        # Preserve the original feature-grid masks for spatial tokens and
        # presence.  Only the support of the masked mean is dilated by one
        # feature voxel, independently for every vessel channel.  Max-pooling
        # binary masks gives the union over the 3x3x3 neighbourhood; channels
        # may intentionally overlap after dilation.
